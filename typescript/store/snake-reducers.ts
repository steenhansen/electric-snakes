...
import cannedMoves from "../game/canned-moves"
import SnakeGame from "../game/SnakeGame"
import project_constants from "../project-constants"
import {EActions, EGameStates, EMoveTypes, EPlayerStates} from "../types/project-enums"
import {IAction} from "../types/redux-interfaces"
import {IHostedGame, IHumanPlayer, IMachinePlayer, IStateStore} from "../types/state-interfaces"

const {HUMAN_PLAYER_NUMBER, INIT_REDUX_ACTION} = project_constants
const {GAME_JOINING_1, GAME_PLAYING_2} = EGameStates
const {PLAYER_PERSON, PLAYER_MACHINE} = EPlayerStates
const {
    TO_SERVER_createGame, TO_SERVER_startPeople, TO_SERVER_moveMachine,
    TO_SERVER_joinGame, TO_SERVER_disconnectBrowser, TO_BROWSER_advanceBoard,
    TO_SERVER_moveSnake, TO_BROWSER_announceWinner,
    TO_BROWSER_announceTie, TO_SERVER_startMachine, TO_BROWSER_crashTurn, TO_BROWSER_announceNames
} = EActions

function createGame_rd(state: IStateStore, action: IAction) {
    const {uuid_key, web_socket, user_name, game_name, register_date} = action
    const register_player: IHumanPlayer = {
         game_name
        , player_number: 1
        , user_name
        , user_turn: 0
    ...
    const register_game: IHostedGame = {
        user_name,
...
function startMachine_rd(state: IStateStore, action: IAction) {
    const machine_game: IHostedGame = {
    ...
    const machine_player: IMachinePlayer = {
        machine_turn: 0
    }
...
function startPeople_rd(state: IStateStore, action: IAction) {
    const hosted_game: IHostedGame = {
        game_date: action.register_date,
        game_against: PLAYER_PERSON,
...
function moveSnake_rd(state: IStateStore, action: IAction) {
    const uuid_key: string = action.uuid_key
    if (action.user_move) {
        const user_move: EMoveTypes = action.user_move
        ...
        const snake_game: SnakeGame = state.current_boards.get(game_name)
...
function moveMachine_rd(state: IStateStore, action: IAction) {
            ...
            if (player_number !== HUMAN_PLAYER_NUMBER) {
                if (typeof machine_board_player !== "undefined") {
                    if (machine_board_player.test_moves.length === machine_board_player.turn_number) {
                        const canned_test_moves = cannedMoves()
                        machine_board_player.test_moves = machine_board_player.test_moves.concat(canned_test_moves)
                    }
                    const the_machine_move: EMoveTypes =
                                  machine_board_player.test_moves[machine_board_player.turn_number]
...
function removeGame(state: IStateStore, game_name: string) {
    const isHumanPlayer = (human_player: (IHumanPlayer|undefined)): boolean => {
    ...
    if (typeof state.human_players !== "undefined") {
        const players_in_game = Seq.Keyed(state.human_players).filter(isHumanPlayer)
        players_in_game.forEach( (human_player, uuid_key): void => {
...
function disconnectBrowser_rd(state: IStateStore, action: IAction) {
    state.human_players = state.human_players.delete(action.uuid_key)
    return state
}

function joinGame_rd(state: IStateStore, action: IAction) {
    const {uuid_key, web_socket, user_name, game_name, player_number} = action
    const join_player: IHumanPlayer = {
        user_name,
        game_name,
        player_number,
        web_socket,
...
function advanceBoard_rd(state: IStateStore, action: IAction) {
    const snake_game: SnakeGame = state.current_boards.get(action.game_name)
    snake_game.advanceBoardTurn()
    state.current_boards = state.current_boards.set(action.game_name, snake_game)
    return state
}

function crashTurn_rd(state: IStateStore, action: IAction) {
    return state
}

function announceNames_rd(state: IStateStore, action: IAction) {
    return state
}

function announceWinner_rd(state: IStateStore, action: IAction) {
    return removeGame(state, action.game_name)
}

function announceTie_rd(state: IStateStore, action: IAction) {
    return removeGame(state, action.game_name)
}

function reducer(state: IStateStore, action: IAction) {
    switch (action.type) {
